module ApplicationHelper
  ...
  def sidebar_link_to(label_class, title, count, path)
    content_tag :li, current_page?(path) ? {:class => 'active'} : {} do
      content_tag :a, :href => path do
        contents = []
        contents << title
        #contents << content_tag(:span, title, :class => "label #{label_class}")
        #contents << content_tag(:span, count, :class => "badge pull-right")
        contents << content_tag(:span, count, :class => "label #{label_class} pull-right")
